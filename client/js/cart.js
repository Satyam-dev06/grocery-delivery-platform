      ...
      '<div class="cart-item-card" data-id="' + p.id + '">' +
        '<a href="product-details.html?id=' + p.id + '" class="cart-item-img-link">' +
          '<div class="cart-item-img-wrap">' +
            '<img src="' + imgUrl(resolveProductImage(p)) + '" alt="' + p.name + '" loading="lazy" onerror="this.style.display=\'none\'">' +
          '</div>' +
        '</a>' +
...
function saveForLater(id) {
  ...
  if (existing === -1) {
    savedList.push({ _id: id, id: id, name: p.name, price: p.price, oldPrice: p.oldPrice, image: resolveProductImage(p), quantity: 1 });
  ...
  savedList.forEach(function (item) {
    ...
    savedItems.innerHTML +=
      '<div class="cart-saved-item">' +
        '<img src="' + imgUrl(resolveProductImage(item) || "") + '" alt="' + (item.name || "") + '" onerror="this.style.display=\'none\'">' +
        '<div class="csi-info">' +
  ...
  prods.forEach(function (p) {
    ...
    container.innerHTML +=
      '<div class="cart-rec-item">' +
        '<img src="' + imgUrl(resolveProductImage(p)) + '" alt="' + p.name + '" onerror="this.style.display=\'none\'">' +
        '<div class="cri-info">' +
          '<h4>' + p.name + '</h4>' +
          '<span class="cri-price">₹' + p.price + '</span>' +
        '</div>' +
        '<button class="cri-add-btn" onclick="addToCartFromEmpty(\'' + pid + '\', \'' + escapeJsStr(p.name) + '\', ' + p.price + ', \'' + escapeJsStr(resolveProductImage(p)) + '\')" aria-label="Add ' + p.name + ' to cart">' +
          '<i class="fas fa-plus"></i>' +
        '</button>' +
      '</div>';
  });
